    ...
    <div class="m-auto p-5">
      ...
      <div id="randomCode">
        <div v-for="item in arrayShopping" class="flex my-5" v-bind:key="item.id">
          ...
          <button class="bg-purple4 mr-3 hover:bg-purple2 text-white font-bold py-2 px-4 rounded" @click="penjumlahan(item.id)">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus" viewBox="0 0 16 16">
              <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z" />
            </svg>
          </button>
          <!--Kurang-->
          <button class="bg-purple5 mr-3 hover:bg-purple2 text-white font-bold py-2 px-4 rounded" @click="pengurangan(item.id)" :disabled="item.jumlah === 0">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-dash" viewBox="0 0 16 16">
              <path d="M4 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 4 8z" />
            </svg>
          </button>
          <!-- Hapus -->
          <button class="bg-red-500 hover:bg-purple2 text-white font-bold py-2 px-4 rounded" @click="hapus(item.id)">
